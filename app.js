(() => {
  ...
  $("#open-invitation-btn").addEventListener("click", () => {
    // 1) curtains sweep IN to cover the screen
    document.body.classList.add("curtains-closing");
    playMusic();
    setTimeout(() => {
      // 2) while hidden behind the curtain, jump to the next section
      $("#greeting").scrollIntoView({ behavior: "auto" });
      // 3) curtains sweep back OUT, revealing the greeting section
      document.body.classList.remove("curtains-closing");
    }, 700);
  });
